...
class SavedEventsViewController: UIViewController {
    ...
    @IBOutlet weak var SvdTitleLbl: UILabel!
    @IBOutlet weak var SvdCityLbl: UILabel!
    @IBOutlet weak var SvdCountryLbl: UILabel!
    @IBOutlet weak var SvdVenueLbl: UILabel!
    @IBOutlet weak var SvdTimeLbl: UILabel!
    @IBOutlet weak var SvdUrlLbl: UILabel!

    @IBOutlet weak var SvdEventLbl: UILabel!
    
    @IBAction func SvdEventSwitch(_ sender: UISwitch) {
        
        // If switch is turned on, event is saved and SaveLbl text is changed        
        if (sender .isOn == true){
            
            selectedEvent?.Save(true)
            SvdEventLbl.text = "Saved event"
        }
        ...
        // If switch is turned off, event is removed from Core Data and SaveLbl text is changed            
            ...
            SvdEventLbl.text = "Save event?"
        }
    }
    
    // Creates variable selectedEvent using info from Event selected in Saved Table View Controller
    ...
    override func viewDidLoad() {
        ...
        SvdTitleLbl.text = selectedEvent?.title
        SvdCityLbl.text = selectedEvent?.city_name
        SvdCountryLbl.text = selectedEvent?.country_name
        SvdVenueLbl.text = selectedEvent?.venue_name
        SvdTimeLbl.text = selectedEvent?.start_time
        SvdUrlLbl.text = selectedEvent?.url
    ...
    // Makes sure clicking chevron next to URL shows Web View
